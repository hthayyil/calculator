/*
 * written by Hanna Thayyil and Larry Tchoingnou
 */
...
//Calibrated ADC values for touch screen; Use sketch from "examples" folder for your touch screen, define your own raw values at the corners here:
#define TS_MINX 138
#define TS_MINY 236
#define TS_MAXX 772
#define TS_MAXY 869

const int TS_LEFT=938,TS_RT=175,TS_TOP=959,TS_BOT=178;
#define MINPRESSURE 200
#define MAXPRESSURE 1000

// For better pressure precision, we need to know the resistance between X+ and X-.
// Use any multimeter to read it. (Shield pins: LCD D6 to LCD RS) or (Arduino pins:D6 to A2)
// If you can not measure this resistance, just keep the nominal value: 300 Ohms.
// (Measured x plate resistance = 267 Ohms.)

TouchScreen ts = TouchScreen(XP, YP, XM, YM, 300);
//TSPoint_kbv tp;

//TFTLCD control pins for shield
//#define LCD_CS A3
//#define LCD_CD A2
//#define LCD_WR A1
//#define LCD_RD A0
...
#define WHITE   0xFFFF
#define DARKRED  0x0000

#define SEGMENT_COLOR_BUFFER_SIZE 10
uint16_t red_color_buffer[SEGMENT_COLOR_BUFFER_SIZE];
uint16_t darkred_color_buffer[SEGMENT_COLOR_BUFFER_SIZE];

/******************* UI details */

//Adafruit_GFX_Button buttons[18];


extern void outputDigit(byte selectDigit, signed char, bool dp);
extern byte readKey();
...
//  const byte rom[] = { // HP35-ROM, 768 words
...
void process_rom(void) { // Process key with HP35-engine
  ...
  //if(display_enable && update_display) lcd_HP35();
  //if(update_display) lcd_HP35();
}
...
//----------------------LCD, SD, TouchScreen Initialize-----------------------
 ...
 tft.setCursor(4,32);
  ...
  delay(70);
  tft.print(" ");
  p.x = map(p.x, 938, 175, 0, tft.width());
  p.y = map(p.y, 959, 178, 0, tft.height());
      do {
  p= ts.getPoint(); 
  pinMode(A2, OUTPUT); //Pins configures again for TFT control
  pinMode(A1, OUTPUT);
   ...
   tft.print("                                            ");
      tft.setCursor(4,28);
   tft.print("                                          ");
   for (int j = 0; j < SEGMENT_COLOR_BUFFER_SIZE; j++) {  //Red and Darkred arrays are to draw digit segments faster with PushColor
    red_color_buffer[j] = RED;                            //1.4 ms per digit with setAddrWindow/PushColor
    darkred_color_buffer[j] = DARKRED;                    //2.4 ms per digit with drawFastVLine/drawFastHLine
   }  
   
   for (int j = -1; j < 15; j++) {  //Blank display
    outputDigit(j, 99, true);    //99: blank segments
   } 
}
...
void lcd_HP35(){
//lcd.clear();
   for (int i = -1; i < 14; i++) {  //Blank display
       outputDigit(i, 99, false);    //99 blank segments
   } 
